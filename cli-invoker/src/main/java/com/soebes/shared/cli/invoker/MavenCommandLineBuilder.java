...
/**
 * @version $Id: MavenCommandLineBuilder.java 1401842 2012-10-24 19:49:47Z
 *          rfscholte $
 */
public class MavenCommandLineBuilder {
	...
	protected void setSettingsLocation(InvocationRequest request,
		...
		if (userSettingsFile != null) {
			try {
				...
				logger.debug(
						"Failed to canonicalize user settings path: "
		...
		if (globalSettingsFile != null) {
			try {
				...
				logger.debug("Failed to canonicalize global settings path: "
	...
	protected void setToolchainsLocation(InvocationRequest request,
		...
		if (toolchainsFile != null) {
			try {
				...
				logger.debug("Failed to canonicalize toolchains path: "
						+ toolchainsFile.getAbsolutePath() + ". Using as-is.",
						e);
			}
	...
	protected void setShellEnvironment(InvocationRequest request,
		...
		if (request.isShellEnvironmentInherited()) {
			try {
				...
				throw new CommandLineConfigurationException(
						"Error reading shell environment variables. Reason: "
								+ e.getMessage(), e);
			} catch (Exception e) {
		...
		if (request.getJavaHome() != null) {
			cli.addEnvironment("JAVA_HOME", request.getJavaHome()
					.getAbsolutePath());
		}
		...
		for (Map.Entry<String, String> entry : request.getShellEnvironments()
				.entrySet()) {
	...
	protected void setProfiles(InvocationRequest request, Commandline cli) {
		...
		if ((profiles != null) && !profiles.isEmpty()) {
			...
			cli.createArg()
					.setValue(StringUtils.join(profiles.iterator(), ","));
		}
	...
	protected void setPomLocation(InvocationRequest request, Commandline cli) {
		...
		if (pomSpecified) {
			try {
				...
				logger.debug("Failed to canonicalize the POM path: " + pom
						+ ". Using as-is.", e);
			}
	...
	protected void setEnvironmentPaths(InvocationRequest request,
		...
		try {
			...
			logger.debug("Failed to canonicalize base directory: "
					+ workingDirectory + ". Using as-is.", e);

			cli.setWorkingDirectory(workingDirectory.getAbsolutePath());
		}

		File localRepositoryDirectory = request
				.getLocalRepositoryDirectory(this.localRepositoryDirectory);
		...
		if (localRepositoryDirectory != null) {
			try {
				...
				logger.debug(
						"Failed to canonicalize local repository directory: "
								+ localRepositoryDirectory + ". Using as-is.",
						e);
			}
			...
			cli.createArg().setValue(
					"maven.repo.local=" + localRepositoryDirectory.getPath());
		}
	}
	...
	protected void setReactorBehavior(InvocationRequest request, Commandline cli) {
		...
		if (StringUtils.isNotEmpty(failureBehavior)) {
			...
			} else if (InvocationRequest.REACTOR_FAIL_NEVER
					.equals(failureBehavior)) {
		...
		if (projectList != null) {
			...
			cli.createArg().setValue(
					StringUtils.join(projectList.iterator(), ","));
	...
	protected void setFlags(InvocationRequest request, Commandline cli) {
		...
		} else if (InvocationRequest.CHECKSUM_POLICY_WARN
	...
	protected File findMavenExecutable()
		...
		if (mavenHome == null) {
			...
			if ((mavenHome == null)
					&& (getSystemEnvVars().getProperty("M2_HOME") != null)) {
		...
		if (mavenExecutable == null || !mavenExecutable.isAbsolute()) {
			...
			try {
				...
				logger.debug("Failed to canonicalize maven executable: "
						+ mavenExecutable + ". Using as-is.", e);
			}

			if (!mavenExecutable.isFile()) {
				throw new CommandLineConfigurationException(
						"Maven executable not found at: " + mavenExecutable);
			}
		}
